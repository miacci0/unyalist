...
// Vercel関数の実行時間上限に対して安全マージンを取るため、待ち時間は短めにしてある
// (以前2000ms/5000msだったが、1件あたりの待ち時間がかさんでFUNCTION_INVOCATION_TIMEOUTの
// 一因になったため短縮した。呼び出し元のlib/gmailSync.jsにも別途、経過時間そのものを
// 見張る安全装置(MAX_RUN_MS)がある)。
const MAX_RETRIES = 2;
const RETRY_DELAYS_MS = [800, 1800];

// fetch()自体にはタイムアウトが無いため、Gemini側の応答が極端に遅い・固まった場合、
// このawaitが数十秒以上戻らずVercel関数の実行時間上限まで居座ってしまう
// (実際にFUNCTION_INVOCATION_TIMEOUTが複数回発生しており、他の対策だけでは防げなかった)。
// 1回の呼び出しごとにAbortControllerで打ち切り、ネットワークエラーと同じ扱いでリトライ/
// あきらめの対象にする。
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    try {
      ...
      if (attempt === MAX_RETRIES) {
        const timeoutErr = new Error(timedOut ? `Gemini API timeout (${REQUEST_TIMEOUT_MS}ms)` : String(err?.message || err));
    ...
    // "quota"/"billing"を含む429は割り当て(日次・月次など)の枯渇で、数秒待っても
    // 解消しないことがほとんどなので、その場でのリトライは行わず即座にあきらめる
    // (呼び出し元がこのメッセージの保存を見送り、後で自動的に再挑戦する。
    // ここで律儀にリトライ待ちをするとVercel関数の実行時間を無駄に消費するだけになる)。
    const isQuotaExhausted = res.status === 429 && /quota|billing/i.test(lastErrText);
    if (!isRetryableStatus(res.status) || isQuotaExhausted || attempt === MAX_RETRIES) {
