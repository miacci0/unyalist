...
// ただし実際には1件あたりの所要時間はネットワーク状況・Geminiのリトライ有無でばらつくため、
// 件数だけで上限を切っても実行時間を確実には抑えられない(実際にFUNCTION_INVOCATION_TIMEOUTが
// 発生した)。件数の上限はあくまで粗い安全網とし、実行時間そのものをMAX_RUN_MSで直接見張る。
export const PROCESS_LIMIT = 20;

// この経過時間(アカウント1件あたり)を超えたら、たとえPROCESS_LIMIT件に達していなくても
// その場でバッチ処理を打ち切る。maxDuration=60に対して十分な安全マージンを取ってある
// (Gmail一覧取得・DB問い合わせ・レスポンス返却など、ループ外の処理時間も考慮)。
...
export async function syncAccount(admin, ownerId, account, threshold) {
  ...
  for (const messageId of idsToProcess) {
    // 件数の上限内でも、経過時間がすでに危険域なら新しいメッセージの処理を始めない
    // (今、着手しているメッセージの処理自体を中断する仕組みはないので、着手前に見て判断する)。
    if (Date.now() - runStartedAt.getTime() > MAX_RUN_MS) {
      stoppedEarly = true;
      break;
    }
    ...
    try {
      ...
      // p_reopen: true により、返信済み/保留/成立の案件はここで「返信待ち」に自動で戻る
      // (今まさに届いた新着返信であるため。lib/threadConsolidation.jsの過去分統合では渡さない)。
...
export async function syncAccounts(admin, ownerId, accounts, threshold) {
  ...
  for (let i = 0; i < settled.length; i++) {
    ...
    if (result.status === "fulfilled") {
      ...
      // 前回までのエラー表示が残っていれば、同期が通った今回でクリアする。
      if (account.last_error) {
        await admin.from("gmail_accounts").update({ last_error: null, last_error_at: null }).eq("id", account.id);
      }
    } else {
      const message = describeSyncError(result.reason);
      results.push({ accountEmail: account.gmail_email, error: message });
      // 画面(アカウント一覧)に警告として出せるよう、アカウント単位で最後のエラーを残す。
      await admin
        .from("gmail_accounts")
...
// アカウント単位の同期エラーを、利用者が次の一手を取れる文言に変換する。
// invalid_grantはrefresh tokenの失効(OAuth同意画面が「テスト」のままだと7日で発生)で、
// 再接続しない限り自然には直らない。
function describeSyncError(err) {
  const raw = String(err?.response?.data?.error || err?.message || err);
  if (/invalid_grant/i.test(raw)) {
    return "Googleの認可が失効しています(invalid_grant)。「再接続」してください";
  }
