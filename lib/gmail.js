import { google } from "googleapis";

// Gmail APIの呼び出しは既定でタイムアウトが無く、応答が極端に遅い場合そのままVercel関数の
// 実行時間上限まで居座ってしまう(実際にFUNCTION_INVOCATION_TIMEOUTの一因になった)。
// googleapisクライアントはgaxios経由なので、各呼び出しの第2引数(GaxiosOptions)に
// timeoutを指定すると、それを超えた時点でエラーとして中断してくれる。
const REQUEST_TIMEOUT_MS = 15_000;

// Gmail読み取り専用アクセス(gmail.readonly)。複数の仕事用Googleアカウントを扱えるよう、
// このモジュールは呼び出し側が復号済みのrefresh tokenを渡してクライアントを作る形にしてある
// (1アカウント固定でモジュール内部にクライアントを持つ実装はやめた)。
// 実際のrefresh token(暗号化済み)はgmail_accountsテーブルに保存され、
// lib/gmailAccountAuth.jsのdecryptRefreshTokenで復号してからここに渡す。

// GOOGLE_CLIENT_ID/GOOGLE_CLIENT_SECRETは全アカウント共通の同一OAuthクライアントを使い回す
// (Googleは同じOAuthクライアントに複数のGoogleアカウントが個別に同意できる)。
export function createGmailClient(refreshToken) {
  const clientId = process.env.GOOGLE_CLIENT_ID;
  const clientSecret = process.env.GOOGLE_CLIENT_SECRET;
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error("GOOGLE_CLIENT_ID / GOOGLE_CLIENT_SECRET / refreshToken が揃っていません");
  }
  const oauth2Client = new google.auth.OAuth2(clientId, clientSecret);
  oauth2Client.setCredentials({ refresh_token: refreshToken });
  return google.gmail({ version: "v1", auth: oauth2Client });
}

// 接続したGoogleアカウント自身のメールアドレスを取得する(OAuth接続時のラベル表示用)。
export async function getProfileEmail(gmail) {
  const { data } = await gmail.users.getProfile({ userId: "me" }, { timeout: REQUEST_TIMEOUT_MS });
  return data.emailAddress || null;
}

// afterUnixSeconds以降に受信したメールのメッセージIDを新しい順に取得する。
// チャット・迷惑メール・ゴミ箱・送信済みは仕事依頼の判定対象から除外する
// (-in:sent: 自分が送った返信そのものは新規の依頼候補になり得ないため。除外しないと
// 返信のたびにGemini分類が走ってしまい、API呼び出しの無駄・誤判定のもとになる)。
// maxResultsはIDの一覧取得だけの上限(軽い呼び出し)なので、遡り日数を増やしても
// 取りこぼさないよう大きめにしてある。実際に本文取得・AI分類まで行う件数は
// lib/gmailSync.jsのPROCESS_LIMITで別途1回の実行あたりに制限している。
export async function listRecentMessageIds(gmail, afterUnixSeconds, { maxResults = 300 } = {}) {
  const q = `after:${Math.floor(afterUnixSeconds)} -in:chats -in:spam -in:trash -in:sent`;
  const messageIds = [];
  let pageToken;
  do {
    const { data } = await gmail.users.messages.list(
      {
        userId: "me",
        q,
        maxResults: Math.min(100, maxResults - messageIds.length),
        pageToken,
      },
      { timeout: REQUEST_TIMEOUT_MS }
    );
    for (const m of data.messages || []) messageIds.push(m.id);
    pageToken = data.nextPageToken;
  } while (pageToken && messageIds.length < maxResults);
  return messageIds;
}

function decodeBase64Url(data) {
  if (!data) return "";
  const normalized = data.replace(/-/g, "+").replace(/_/g, "/");
  return Buffer.from(normalized, "base64").toString("utf-8");
}

function stripHtml(html) {
  return html
    .replace(/<style[\s\S]*?<\/style>/gi, " ")
    .replace(/<script[\s\S]*?<\/script>/gi, " ")
    .replace(/<br\s*\/?>/gi, "\n")
    .replace(/<\/(p|div|tr|li|h[1-6])>/gi, "\n")
    .replace(/<[^>]+>/g, " ")
    .replace(/&nbsp;/g, " ")
    .replace(/&amp;/g, "&")
    .replace(/&lt;/g, "<")
    .replace(/&gt;/g, ">")
    .replace(/[ \t]+/g, " ")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
}

// text/plainパートを優先し、無ければtext/htmlを簡易タグ除去してプレーンテキスト化する。
// 追加npm依存を増やさないための軽量実装(完璧な変換は狙わない)。
function extractBody(payload) {
  if (!payload) return "";

  let plainText = null;
  let htmlText = null;

  function walk(part) {
    if (!part) return;
    const mimeType = part.mimeType || "";
    if (mimeType === "text/plain" && part.body?.data && plainText === null) {
      plainText = decodeBase64Url(part.body.data);
    } else if (mimeType === "text/html" && part.body?.data && htmlText === null) {
      htmlText = decodeBase64Url(part.body.data);
    }
    for (const child of part.parts || []) walk(child);
  }
  walk(payload);

  if (plainText !== null) return plainText;
  if (htmlText !== null) return stripHtml(htmlText);
  return "";
}

function headerValue(headers, name) {
  const header = (headers || []).find(h => h.name.toLowerCase() === name.toLowerCase());
  return header?.value || "";
}

// "山田太郎 <taro@example.com>" 形式のFromヘッダーを{name, email}に分解する。
function parseFrom(fromHeader) {
  const match = fromHeader.match(/^(.*?)<([^>]+)>$/);
  if (match) {
    const name = match[1].trim().replace(/^"|"$/g, "");
    return { name: name || match[2].trim(), email: match[2].trim() };
  }
  return { name: fromHeader.trim(), email: fromHeader.trim() };
}

// スレッドIDだけを軽量に取得する(lib/threadConsolidation.jsが、スレッド機能導入より前に
// 保存された既存のinquiries行へthread_idを後付けする際に使う)。format:"minimal"は
// id/threadId/labelIdsのみを返すため、本文・ヘッダーを取得するgetMessageDetailより軽い。
export async function getMessageThreadId(gmail, messageId) {
  const { data } = await gmail.users.messages.get(
    { userId: "me", id: messageId, format: "minimal" },
    { timeout: REQUEST_TIMEOUT_MS }
  );
  return data.threadId || null;
}

// 指定したメッセージIDの件名・送信者・本文プレーンテキスト・受信日時・スレッドIDを取得する。
// threadIdは同じやり取り(元の依頼メール+その返信すべて)に共通のIDで、format:"full"の
// レスポンスに追加の呼び出しなしで含まれている(lib/gmailSync.jsのスレッド判定に使う)。
export async function getMessageDetail(gmail, messageId) {
  const { data } = await gmail.users.messages.get(
    { userId: "me", id: messageId, format: "full" },
    { timeout: REQUEST_TIMEOUT_MS }
  );
  const headers = data.payload?.headers || [];
  const { name: senderName, email: senderEmail } = parseFrom(headerValue(headers, "From"));
  const subject = headerValue(headers, "Subject");
  const bodyText = extractBody(data.payload);
  const receivedAt = data.internalDate ? new Date(Number(data.internalDate)).toISOString() : new Date().toISOString();

  return {
    messageId,
    threadId: data.threadId || null,
    senderName,
    senderEmail,
    subject,
    bodyText,
    receivedAt,
  };
}
