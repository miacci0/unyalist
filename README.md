...
2. OAuth同意画面を設定する(External)。**公開ステータスは必ず「本番環境」にする(「アプリを公開」)。**
   ...
   (先に接続すると、そのtokenはテスト扱いで7日で失効する)。公開には同意画面(ブランディング)に
   次の値の設定が必要: アプリのホームページ `https://unyalist.vercel.app`、
   プライバシーポリシー `https://unyalist.vercel.app/privacy`(本アプリが公開ページとして提供。
   実装は`app/privacy/page.js`)、承認済みドメイン `unyalist.vercel.app`、ユーザーサポートメール。
   ポリシーの記載はコードの実挙動に合わせてあるため、取得・保存する情報や外部サービスを
   変更したら`app/privacy/page.js`も更新すること。なお、やむを得ず「テスト」のまま
...
毎回処理する)ため、思い出したときにいつでも実行してよい(この統合だけではステータスは
変化しない。下記「返信待ち」の自動遷移は新着返信の取り込み時のみ働く)。
